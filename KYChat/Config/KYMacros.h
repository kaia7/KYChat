...
#pragma mark - # Methods
#define     KYURL(urlString)    [NSURL URLWithString:urlString]
#define     KYNoNilString(str)  (str.length > 0 ? str : @"")
#define     KYWeakSelf(type)    __weak typeof(type) weak##type = type;
#define     KYStrongSelf(type)  __strong typeof(type) strong##type = type;
#define     KYTimeStamp(date)   ([NSString stringWithFormat:@"%lf", [date timeIntervalSince1970]])
#define     KYColor(r, g, b, a) [UIColor colorWithRed:(r)/255.0f green:(g)/255.0f blue:(b)/255.0f alpha:a]
